...
4. First try the verified read route suitable for the detected client. WeChat 4.1.11+ commonly requires startup capture. Generate an anchor from the installed DLL; never reuse an RVA when the DLL SHA-256 differs.
5. If startup capture is required, prepare the command before asking the user to exit WeChat. The user performs login and any phone confirmation. Accept a candidate only when that database's page-1 HMAC verifies. On every exit path, unload instrumentation and wipe held mutable key buffers.
...
8. Run `scripts/run.ps1 render <output-directory>`. Inspect the offline HTML at desktop and mobile width, expand at least one source, and view every PNG part. Confirm statistics, source IDs, no network dependencies, Chinese fonts, full height, and temporary plaintext count zero.
